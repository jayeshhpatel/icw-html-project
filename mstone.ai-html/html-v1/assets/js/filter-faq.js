	...
	$doc.ready( function() {
			...
			$.ajax({
				...
				complete: function(data, textStatus) {
					...
					if (msg === 'No posts found') {
						$content.html('<div class="text-center pt-50 pb-50 w-100"><div class="alert alert-danger">No questions found</div></div>');
					}
		...
		$(document).on('click', '#container-async a[data-filter], #container-async .pagination a', function(event) {
			...
			if ($(this).data('infscr') === 'load') {
				$(this).html('<span class="arrow-icon icw-loading"></span><span class="btn-text">Load Questions</span>');
			}
