...
var factory = function($, window) {
  var ICWAutocomplete = function(el, options) {
  ...
  ICWAutocomplete.prototype = {
    ...
    setupMarkup: function() {
      ...
      this.field.parents('.search-block').addClass(this.settings.wrapClasses);
      // this.el = $('.search-list'); //this.field.parent();

      this.el = this.field.parents('.search-block');
    },
    ...
    renderGroups: function() {
      ...
      // console.log(this.el);
      this.el.find('.search-list').append(this.list);
    },
  ...
  ICWAutocomplete.defaults = ICWAutocomplete.prototype.defaults;

  $.fn.icwAutocomplete = function(settings) {
    return this.each(function() {
      if (this.icwAutocomplete) {
        // Prevent ICWAutocomplete from creating a new object. Instead,
        // just update the settings object
        $.extend(this.icwAutocomplete.settings, settings);
        return this;
      }

      var d = new ICWAutocomplete(this, settings).init();

      // Expose "icwAutocomplete.settings" to the cold outside
      this.icwAutocomplete = { settings: d.settings };
    });
  };
};
