(function($) {
	...
	$doc.ready( function() {
		 ...
		 */
		$(document).on('click', '#container-async a[data-filter], #container-async .pagination-pager a', function(event) {
			...
			if ($this.data('filter')) {
				...
				if ($this.data('term') === 'all-terms') {
					$this.closest('ul.nav-filter').find('.active').removeClass('active');
					$('#q').val('');
				}
				else {
				 ...
				 */
				$active = {};
				$terms  = $this.closest('ul.nav-filter').find('.active');
			...
			if ($(this).data('infscr') === 'load') {
				$(this).html('<span class="icw-loading"></span> Load tip posts');
			}
			
			if ($(this).hasClass('page-numbers')) {
				$('html, body').stop().animate({
					scrollTop: $('#container-async').offset().top - 180
				}, 100);
			}
