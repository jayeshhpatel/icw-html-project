	...
	$doc.ready( function() {
		...
		function get_posts($params) {
			...
			$.ajax({
				url: icw.ajax_url,
				data: {
					action: 'do_filter_resources_cat',
					nonce: icw.nonce,
					params: $params,
					pager: $method
				},
				...
				complete: function(data, textStatus) {
					...
					if (msg === 'No posts found') {
						$content.html('<li>&nbsp;</li><li class="text-center pt-50 pb-50"><div class="alert alert-danger">No resource found</div></li><li>&nbsp;</li>');
					}
		...
		$(document).on('click', '#container-async a[data-filter], #container-async .pagination a', function(event) {
			...
			$params    = {
				...
				'catid'   : $this.closest('#container-async').data('catid'),
				'q' 	: $('#q').val()
			};
